...
class Register:
    ...
    def validation(self):
        if self.name_var.get() == '':
            ...
            try:
                conn = mysql.connector.connect(host='localhost', username='root', password='Abhijeet@15', database='Register')
                my_cursor = conn.cursor()
                my_cursor.execute('INSERT INTO Data (name, email, contact, gender, country, id_type, id_number, password) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)', (
                    self.name_var.get(),
                    self.email_var.get(),
